...
from sheeprl.algos.dreamer_v3_mod.agent import WorldModel, build_agent
from sheeprl.algos.dreamer_v3_mod.loss import reconstruction_loss
from sheeprl.algos.dreamer_v3_mod.utils import Moments, compute_lambda_values, prepare_obs, test
...
def train(
    ...
    observed_area_ratio = data["observed_area_ratio"] if "observed_area_ratio" in data else None
    ...
    if cfg.algo.world_model.decoupled_rssm:
        ...
        for i in range(0, sequence_length):
            ...
            recurrent_state, posterior_logits, prior_logits = world_model.rssm.dynamic(
                posterior,
                recurrent_state,
                batch_positions[i : i + 1], # action改为position输入
                data["is_first"][i : i + 1],
            )
    ...
    # 训练actor时使用imagination进行训练，而不使用真实的后验概率
    ...
    policy_loss = -torch.mean(discount[:-1].detach() * (objective + entropy.unsqueeze(dim=-1)[:-1])) # 策略损失函数
...
def main(fabric: Fabric, cfg: Dict[str, Any]):
    ...
    for iter_num in range(start_iter, total_iters + 1):
        ...
        with torch.inference_mode():
            ...
            for k in obs_keys:
                step_data[k] = next_obs[k][np.newaxis] # 将next_obs的每个元素添加到step_data中
