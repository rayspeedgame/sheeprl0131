...
from sheeprl.algos.dreamer_v3_mod.utils import init_weights, uniform_init_weights
...
class RSSM(nn.Module):
    ...
    def dynamic(
        ...
        recurrent_state = self.recurrent_model(posterior, recurrent_state) # 去除action的输入
        prior_logits, prior = self._transition(recurrent_state)
        posterior_logits, posterior = self._representation(recurrent_state, embedded_obs, action) # 增加action输入 改position
    ...
    def _representation(self, recurrent_state: Tensor, embedded_obs: Tensor, action: Tensor) -> Tuple[Tensor, Tensor]:
        """
        Args:
        ...
        """
        logits: Tensor = self.representation_model(torch.cat((recurrent_state, embedded_obs, action), -1)) # 增加action输入
    ...
    def imagination(self, prior: Tensor, recurrent_state: Tensor) -> Tuple[Tensor, Tensor]:
        """
        ...
        """
        recurrent_state = self.recurrent_model(prior, recurrent_state) # 去除action输入
...
def build_agent(
    ...
    if hasattr(cfg.env, 'screen_size'):
        ...
        # 对于UAVenv等自定义环境，使用观测空间的实际尺寸
        # 假设所有CNN输入具有相同的空间尺寸
        if cfg.algo.cnn_keys.encoder and len(cfg.algo.cnn_keys.encoder) > 0:
            obs_height = obs_space[cfg.algo.cnn_keys.encoder[0]].shape[-2]
            # 计算从obs_height降采样到4x4所需的stages数
            # 每个stage将尺寸减半，所以需要log2(obs_height/4)个stages
            cnn_stages = max(1, int(np.log2(obs_height) - np.log2(4)))
        else:
            # 默认值，如果没有CNN输入
            cnn_stages = 4
    cnn_encoder = (
    ...
    represention_model_input_size += sum(actions_dim) # represention_model_input_size增加action尺寸
    ...
    reward_ln_cls = hydra.utils.get_class(world_model_cfg.reward_model.layer_norm.cls) # 添加action输入
    reward_model = MLP(
        input_dims=latent_state_size + sum(actions_dim),
